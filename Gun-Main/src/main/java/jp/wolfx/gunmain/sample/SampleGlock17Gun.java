package jp.wolfx.gunmain.sample;
...
public class SampleGlock17Gun implements CustomGun {
    ...
    @Override public String getId() { return "sample:glock17"; }
    @Override public String getName() { return "§bGlock 17 (9mm)"; }
    ...
    public ItemStack craftItemStack() {
        ...
        meta.setCustomModelData(1006); // グロック専用モデルデータ
    ...
    public void onShoot(Player player, ItemStack gunItem) {
        ...
        if (ammo == null || ammo <= 0) {
            ...
            player.sendMessage("§cGlock 17 弾切れです！ (9mm必要)");
            return;
        }
        ...
        player.playSound(player.getLocation(), Sound.ENTITY_FIREWORK_ROCKET_BLAST, 1.2f, 1.8f);
        RayTraceResult result = player.getWorld().rayTraceEntities(player.getEyeLocation(), player.getEyeLocation().getDirection(), 40.0, 0.2, e -> e != player);
    ...
    public void onReload(Player player, ItemStack gunItem) {
        player.playSound(player.getLocation(), Sound.BLOCK_IRON_TRAPDOOR_CLOSE, 1.0f, 1.4f);
        player.sendMessage("§eGlock 17 マガジン交換中...");
        org.bukkit.Bukkit.getScheduler().runTaskLater(GunMainPlugin.getInstance(), () -> {
            if (player.isOnline() && player.getInventory().getItemInMainHand().equals(gunItem)) {
                ...
                if (meta != null) {
                    meta.getPersistentDataContainer().set(ammoKey, PersistentDataType.INTEGER, getMaxAmmo());
                    gunItem.setItemMeta(meta);
                    player.playSound(player.getLocation(), Sound.BLOCK_IRON_TRAPDOOR_OPEN, 1.0f, 1.5f);
                    player.sendMessage("§aGlock 17 リロード完了！");
                }
            }
        }, 25L);
    }
